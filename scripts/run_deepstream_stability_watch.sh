...
# 사용:
#   ./scripts/run_deepstream_stability_watch.sh [실행시간(분)] [간격(초)] [로그파일]
# 예시:
...
usage() {
    cat <<'EOF'
Usage:
  ./scripts/run_deepstream_stability_watch.sh [duration_min] [interval_sec] [log_file]
...
run_check "smoke test script exists" test -f scripts/smoke_test_data_flow.py || PRECHECK_FAILED=1
