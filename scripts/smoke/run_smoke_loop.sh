...
# 사용: ./scripts/smoke/run_smoke_loop.sh [실행시간(분)] [간격(초)]
# 예시: ./scripts/smoke/run_smoke_loop.sh 60 30   → 60분간 30초 간격으로 실행
...
while [[ $(date +%s) -lt $END ]]; do
    ...
    RESULT=$(python scripts/smoke/smoke_test_data_flow.py 2>&1)
