...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="외부 MQTT 입력을 수신해 내부 이벤트로 정규화합니다."
    )
    parser.add_argument("--mqtt-broker", help="외부 MQTT 브로커 호스트")
    parser.add_argument("--mqtt-port", type=int, help="외부 MQTT 브로커 포트")
    parser.add_argument(
        "--topic", action="append", dest="topics", help="구독할 토픽. 여러 번 지정 가능"
    )
    parser.add_argument(
        "--mqtt-client-id",
        help="외부 MQTT client id. 브로커가 고정 ID를 요구할 때 사용",
    )
    ...
    parser.add_argument(
        "--republish",
        action="store_true",
        help="정규화 이벤트를 내부 MQTT 토픽으로 재발행",
    )
