...
_ALERT_LOG = Path(os.environ.get("ALERT_LOG_PATH", "/app/alert_api_events.jsonl"))


def _read_events(limit: int, offset: int, camera_id: Optional[str]) -> tuple[List[EventOut], int]:
                ...
                all_items.append(
                    {
                        ...
                        "event_type": payload.get("type", payload.get("event_type", "other")),
...
@router.get(
    ...
    description="최신 순으로 탐지 이벤트를 반환합니다. camera_id로 필터링 가능합니다.",
)
def list_events(
    ...
    items, total = _read_events(limit, offset, camera_id)
