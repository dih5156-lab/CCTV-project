"""센서 / 경광등(Siren) 디바이스 컨트롤러

인수인계 자료의 WarnbellOption DTO 구조를 참고하여 작성한 모듈이다.

동작 흐름 (trigger):
  ① POST /interm-api/Warnbell/Control  - 경광등 ON
  ② (자동 꺼짐 없을 경우) Timer로 stop() 호출
"""
...
class SensorConfig:
    ...
    속성:
        ...
        username:         인증 사용자명
        password:         인증 비밀번호
        auto_stop_seconds: 자동 정지까지 대기 시간 (0=자동 정지 없음)
    ...
    username: str = ""
    password: str = ""
    auto_stop_seconds: float = 10.0   # 기본 10초 후 자동 정지
