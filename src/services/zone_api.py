"""zone_api.py - 위험구역 및 구역 프리셋 REST API 서버.

카메라별 위험구역(폴리곤/라인)과 구역 프리셋을 조회·저장·삭제하는 경량 HTTP 서버.
...
class ZoneApiHandler(BaseApiHandler):
    """카메라 위험구역 및 프리셋 REST API 핵들러.
...
def start_zone_api_server(
    ...
    """
    server = ThreadingApiServer(("0.0.0.0", port), ZoneApiHandler)
