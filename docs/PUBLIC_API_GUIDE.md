...
| `ALERT_LOG_PATH` | `/app/logs/alert_api_events.jsonl` | 이벤트 조회 API가 읽는 JSONL 로그 |
| `ALERT_FALLBACK_LOG` | `/app/logs/public_api_fallback.jsonl` | Alert API 중계 실패 시 fallback 로그 |
| `CAMERAS_JSON` | `/app/cameras.json` | 카메라 목록 조회에 사용하는 설정 파일 |
| `APPEARANCES_DB` | `/app/data/appearances.db` | 외형 조건/검색에 사용하는 SQLite DB |
...
- [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md): 전체 프로젝트 구조와 데이터 흐름
...
- [ACTION_LAYER_SPEAKER_BRIDGE.md](ACTION_LAYER_SPEAKER_BRIDGE.md): Action Layer와 알람 장치 연동
- [KUIPER_RULE_ENGINE.md](KUIPER_RULE_ENGINE.md): eKuiper 룰 엔진 구성
