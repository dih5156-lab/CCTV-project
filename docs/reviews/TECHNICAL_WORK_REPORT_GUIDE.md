...
- .venv/bin/python scripts/health/check_jetson_edgex_stack.py --host localhost --public-api-port 9000 --deepstream --check-appearance-status
...
- `config/deepstream/*`
- `scripts/health/check_jetson_edgex_stack.py`
...
| 스크립트 | 목적 |
|---|---|
| `scripts/smoke/smoke_test_deployment.py` | health/readiness 중심 배포 점검 |
| `scripts/smoke/smoke_test_data_flow.py` | Alert API, Action Layer, Public API 데이터 흐름 점검 |
| `scripts/health/check_sensitive_defaults.py` | 민감 기본값 유출 점검 |
| `scripts/health/check_jetson_edgex_stack.py` | Jetson/EdgeX/Action Layer 현장 점검 |
| `scripts/health/check_model_report.py` | 모델 평가 리포트 기준 확인 |
| `scripts/health/check_offline_readiness.py` | 오프라인 배포 준비 상태 확인 |
