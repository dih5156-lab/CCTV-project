...
├── run_external_ingest.py          # 외부 MQTT 수신 호환 진입점
├── app/                            # 실제 CLI 구현(main, external ingest)
...
AI Engine의 하위 호환 진입점입니다. 실제 CLI 구현은 `app/main.py`에 있고 루트 `main.py`가 이를 호출합니다.
...
├── detection_snapshot_store.py     # 감지 스냅샷 저장
├── event_debouncer.py              # 이벤트 발행 간격 제어
├── event_dispatcher.py             # 이벤트 목적지별 전달
├── _adaptive_governor.py           # 성능/부하 적응 제어
├── _camera_registry.py             # 카메라 등록, 스레드, 재연결
├── _context_event_store.py         # 얼굴·외형 컨텍스트 이벤트 임시 저장
├── _deepstream_context_worker.py   # 얼굴·외형 분석 비동기 worker
├── _deepstream_element_config.py   # GStreamer element 설정과 링크 검증
├── _deepstream_env.py              # 환경변수 파싱과 범위 보정
├── _deepstream_event_factory.py    # DeepStream event 생성
├── _deepstream_event_queue.py      # 이벤트 큐와 기존 필터 파이프라인 연결
├── _deepstream_face_context.py     # DeepStream 얼굴 컨텍스트
├── _deepstream_labels.py           # 추론 라벨과 EventType 변환
├── _deepstream_model_flags.py      # 카메라 모델 플래그 정규화
├── _deepstream_osd.py              # OSD와 낙상 skeleton overlay
├── _deepstream_pipeline_builder.py # pipeline 생성, 연결, 런타임 hook
├── _deepstream_source_health.py    # source 오류, 재시도, 재시작 판단
├── _deepstream_source_manager.py   # 런타임 source attach/detach
├── _deepstream_source_state.py     # source 상태와 pad-camera 매핑
├── _deepstream_tensor_utils.py     # tensor metadata 디코딩
├── _deepstream_topology.py         # 카메라 기능별 추론 topology 계산
...
├── _fall_shadow_review.py          # 낙상 shadow 기록과 비동기 검증 요청
├── _h264_poc_fixer.py              # H.264 POC buffer 보정
├── _inference_pipeline.py          # AI 추론과 이벤트 큐 적재
├── _preview_frame_store.py         # Stream API용 최신 preview frame 저장
...
├── _action_bridge_alarm.py         # 알람 대상 판정과 쿨다운
├── _action_bridge_executor.py      # 장비 제어, 외부 전달, 결과 저장
├── _action_bridge_models.py        # 제어 모드, 장비, 사이트 모델
├── _action_bridge_payloads.py      # 센서 payload 정규화
├── _action_bridge_repo.py          # action_events SQLite 저장소
├── _action_bridge_rest_queue.py    # REST 이벤트 비동기 큐
├── _action_bridge_site_registry.py # 사이트별 모드와 장비 설정
├── _action_bridge_topics.py        # 구독·알람·명령 토픽 정의
├── _action_bridge_support.py       # 하위 호환 import와 공통 지원 객체
├── _device_reachability.py         # 알람 장비 연결 가능 여부 캐시
...
├── external_ingest.py              # 외부 MQTT ingest
...
├── siren.py                        # HTTP 경광등/사이렌
├── speaker.py                      # HTTP Digest 스피커
└── signboard.py                    # TCP 전광판
```
...
├── http_outbox.py                  # 외부 HTTP 재전송 SQLite outbox
├── mqtt_outbox.py                  # MQTT 재전송 SQLite outbox
...
현재 시연에서는 Compose의 `public-demo-ui`가 제공하는 `http://localhost:7000/public-demo.html`을 우선 사용합니다. `web/public-demo.html`은 해당 화면의 소스 파일입니다.
...
1. `README.md`
2. `docs/modules/PROJECT_STRUCTURE.md`
