...
이 문서의 초기 권장안 중 다음 항목은 현재 코드에 반영되어 있습니다.

| 영역 | 현재 구현 |
|---|---|
| 외부 ingest | `ingest_events.event_id`와 unique index로 중복 저장 방지 |
| EdgeX outbox | `event_outbox`에 destination, 상태, retry, 만료 시각과 `(event_id, destination_name)` unique index 적용 |
| MQTT outbox | `mqtt_event_outbox`에 event/destination 기준 중복 방지와 FIFO 재처리 상태 저장 |
| HTTP outbox | `http_event_outbox`에 event/target URL 기준 중복 방지와 재시도 상태 저장 |
| Action Layer | `action_events.event_id` unique, 장비 실행 및 HTTP 전송 결과 저장 |
| 외형 로그 | `appearance_log.event_id`, `attribute_backend`, `attribute_metadata`와 검색 인덱스 적용 |
| 이벤트 검수 | `event_reviews`에 event별 운영자 판정 저장 |

아직 공통 `schema_version` 컬럼을 모든 SQLite 테이블에 강제하거나, 모든 outbox 구현을 하나의 물리 테이블로 합치지는 않았습니다. 서로 다른 전송 채널은 현재 별도 테이블과 DB 파일을 사용합니다.
...
- `event_id` 컬럼과 unique index
- `attribute_backend`, `attribute_metadata` 컬럼
- timestamp, camera, 색상, 헬멧, 성별 검색 인덱스

남은 권장 사항:

- 필요하면 `schema_version` 컬럼 추가
...
현재 구현은 `src/canonical_event.py`에서 카메라, 이벤트 타입, 발생 시각, 객체 ID와 payload를 조합해 SHA-1 기반 안정 ID를 생성합니다. 사람이 읽기 쉬운 권장 표현은 다음과 같습니다.
...
- 필요한 테이블에 `schema_version`을 추가하고 마이그레이션 기준 정의
- `event_outbox`, `http_event_outbox`, `mqtt_event_outbox`의 공통 상태·오류 필드 규약 정리
