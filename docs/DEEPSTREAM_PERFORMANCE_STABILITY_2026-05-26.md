...
- 단, 이번 재검증에서 `scripts/check_jetson_edgex_stack.py`의 일부 항목은 실패했습니다. 실패 원인은 DeepStream 자체 장애라기보다 호스트 기준 포트/인증/런타임 위치 차이로 보입니다.
...
```bash
./scripts/run_smoke_loop.sh 3 30
```
...
```bash
.venv/bin/python scripts/check_jetson_edgex_stack.py --host localhost --public-api-port 9000 --deepstream --check-appearance-status --json
```
...
| 데이터 흐름 | `scripts/smoke_test_data_flow.py` 반복 통과 | stability watch 로그 |
