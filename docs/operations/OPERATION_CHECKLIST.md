...
- 변수 기준표는 `docs/operations/DEPLOYMENT_ENVIRONMENT_VARIABLES.md`를 따른다
...
```bash
sudo docker compose -f docker-compose.jetson.yml up -d --build --force-recreate cctv-public-api
RUNTIME_ENV_FILE=.env.jetson ./scripts/ops/run_operation_check.sh
```
...
- 최종 판단은 `docs/operations/DEEPSTREAM_PERFORMANCE_STABILITY_2026-05-26.md`의 표준 안정성 통과 기준을 따름
...
sudo RUNTIME_ENV_FILE=.env.jetson ./scripts/ops/run_operation_check.sh
```
