...
```bash
python scripts/health/check_jetson_edgex_stack.py --host 127.0.0.1
```

외형 검색 상태까지 포함:

```bash
python scripts/health/check_jetson_edgex_stack.py \
  --host 127.0.0.1 \
...
[../api/APPEARANCES_STATUS_API.md](../api/APPEARANCES_STATUS_API.md)
...
```bash
python scripts/health/check_jetson_edgex_stack.py --json
```
